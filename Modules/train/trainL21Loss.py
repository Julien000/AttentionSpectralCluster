...
def train_SpectralNet_attNet( train_dataloader, test_dataloader, model_ae, args, model_siam=None):
  ...
  # criterion = L21LossV3(beta=args.beta) # 不要用这个loss，效果不好。
  criterion = L21LossV2(beta=args.beta) # 效果最好
  # criterion = L21LossV4(beta=args.beta) # 与L21LossV2一样， 即便是L_{2,1}的梯度按1.0/2()来计算，也能达到0.946，2023-08-11
  ...
  for epoch in range(args.epochs):
    ...
    for zipdata in tqdm(train_dataloader, desc=f'train_Epoch [{epoch+1}/{args.epochs}]',total=train_length) :
        ...
        Y.backward(gradient=grads) #，要用loss的backward()
        # train_loss.backward() #更新梯度
        optimizer.step()
        # loss_t += torch.abs(torch.sum(grads)/W.shape[0]).item()
        loss_t += train_loss.item()/W.shape[0]
    ...
    for  x, tar in test_dataloader:
      ...
      val_loss, grad = criterion(Y, W, isVal=True)
